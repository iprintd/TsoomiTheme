...
class KitItem extends Component {
  ...
  componentDidMount() {
    $('#kit-continue-button').css({'background':'#f1f1f3', 'border':'1px solid #f1f1f3'});
    
    var globalsum = 0;
    $('.quantity-control').each(function (){
      globalsum = globalsum + parseInt($(this).val());
    });
    console.log(globalsum);

    $(document).on("keyup change", ".quantity-control", function (){
      var sum = 0;
      $('.quantity-control').each(function (){
      ...
      if(sum<100) {
        ...
        $('#kit-continue-button').css({'background':'#008080', 'border':'1px solid #008080'});
       } 
       console.log(globalsum);
    });

    $('#kit-continue-button').on("click", function() {
      console.log(globalsum);
      if(globalsum<100) {
  ...
  renderEditLink(className, isOutOfStock) {
    ...
    return (
      <div className={'edit-section ' + className}>
        ...
        {kitItemOrderItemModel.IsIncluded && !kitItemViewModel.iframeLoading && !isOutOfStock && (<span className={'edit-options'} onClick={this.onClickEditOptions}><ResponsiveEllipsis style={{ whiteSpace: 'pre-wrap' }} text={'העלאת קובץ'/*t('KitItem.Edit_options')*/} maxLine={2} basedOn='words' /></span>)}
        {(!kitItemOrderItemModel.IsIncluded || isOutOfStock) && (<span className='disabled-edit-options'><ResponsiveEllipsis style={{ whiteSpace: 'pre-wrap' }} text={'העלאת קובץ'/*t('KitItem.Edit_options')*/} maxLine={2} basedOn='words' /></span>)}
      </div>
    )
  }
    ...
    return (
      ...
      <div className={kitItemClasses}>
        ...
        <Modal isOpen={this.state.isPropertiesOpen} modalClassName="kit-item-properties-modal" onOpened={this.propertiesModalOnOpened}>
          <ModalHeader>{`${'העלאת קובץ'/*t('KitItem.Edit_options')*/} - ${Name}`}<div className='modal-close'><div className='close-btn' onClick={this.onClickCancel}>×</div></div></ModalHeader>
          <ModalBody>
            {
